...
import { showConfirm, showToast } from '../../app.js?v=8.32';
import { renderCurrentView } from '../../components.js?v=8.32';
import { archiveDiscipline, unarchiveDiscipline } from '../../logic.js?v=8.32';
import { scheduleSave } from '../../store.js?v=8.32';
import {
  openEditaModal,
  saveEdital,
  deleteEdital,
...
import {
  ...
  openMatchCorrector,
  saveMatchCorrection,
} from '../../views/banca-view.js?v=8.32';

// Registrar ações
registerAction('navigate', (el) => {
  const view = el.dataset.view;
  if (view) import('../../app.js?v=8.32').then(({ navigate }) => navigate(view));
});
...
registerAction('save-disc-manager', (el) => {
  ...
  const discId = el.dataset.discId;
  if (editalId && discId) {
    import('../../views.js?v=8.32').then(({ saveDiscManager }) =>
...
registerAction('move-subject', (el) => {
  ...
  if (discId)
    import('../../views.js?v=8.32').then(({ moveSubject }) => moveSubject(discId, idx, dir));
});
