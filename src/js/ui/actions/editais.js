...
registerAction('navigate', (el) => {
  ...
  if (view) import('../../app.js?v=8.37').then(({ navigate }) => navigate(view));
});
...
registerAction('save-disc-manager', (el) => {
  ...
  const discId = el.dataset.discId;
  if (editalId && discId) {
    import('../../views.js?v=8.37').then(({ saveDiscManager }) =>
...
registerAction('move-subject', (el) => {
  ...
  const dir = parseInt(el.dataset.dir, 10);
  if (discId)
    import('../../views.js?v=8.37').then(({ moveSubject }) => moveSubject(discId, idx, dir));
});
