...
import { archiveDiscipline, unarchiveDiscipline } from '../../logic.js?v=8.30';
import { scheduleSave } from '../../store.js?v=8.30';
import {
  openEditaModal,
  saveEdital,
  deleteEdital,
...
import {
  ...
  openMatchCorrector,
  saveMatchCorrection,
} from '../../views/banca-view.js?v=8.30';

// Registrar ações
registerAction('navigate', (el) => {
  const view = el.dataset.view;
  if (view) import('../../app.js?v=8.30').then(({ navigate }) => navigate(view));
});
...
registerAction('save-disc-manager', (el) => {
  ...
  const discId = el.dataset.discId;
  if (editalId && discId) {
    import('../../views.js?v=8.30').then(({ saveDiscManager }) =>
...
registerAction('move-subject', (el) => {
  ...
  if (discId)
    import('../../views.js?v=8.30').then(({ moveSubject }) => moveSubject(discId, idx, dir));
});
