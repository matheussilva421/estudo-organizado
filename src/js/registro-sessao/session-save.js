...
function findNextPendingPlanningSequenceForDisc(discId) {
  if (!discId || !state.planejamento?.ativo || !Array.isArray(state.planejamento.sequencia)) {
    return null;
  }
  return (
    state.planejamento.sequencia.find(
      (seq) => seq.discId === discId && getSeqStatus(seq) === 'pendente'
    ) || null
  );
}

function shouldLinkManualSessionToPlanning(currentEventId, ev) {
  if (ev?.seqId) return false;
  const isManualSession = currentEventId === 'crono_livre' || !ev?.isAutoGenerated;
  return isManualSession && !!document.getElementById('reg-vincular-planejamento')?.checked;
}
...
export function performSave({
  ...
  const linkedManualSeq = shouldLinkManualSessionToPlanning(currentEventId, ev)
    ? findNextPendingPlanningSequenceForDisc(discId)
    : null;
  ...
  if (isLivre && discId) {
    ...
    const evtReal = {
      ...
      ...(linkedManualSeq ? { seqId: linkedManualSeq.id } : {}),
    };
  ...
  if (!ev.seqId && linkedManualSeq) {
    ev.seqId = linkedManualSeq.id;
  }
