...
## Nota de Revisao Posterior - 2026-05-03

Revisao tecnica sobre este rewrite encontrou duas inconsistencias relevantes para estabilidade de sync:

1. `SyncLock` nao deve ser reentrante por heuristica global. A implementacao anterior tratava qualquer chamada enquanto o lock estava ocupado como "mesmo owner", permitindo concorrencia real dentro da secao critica. A correcao remove a reentrancia implicita, adiciona `primarySyncLock` separado para o coordenador e usa helpers internos sem lock onde havia chamadas aninhadas intencionais.
2. `saveStateToDB()` deve emitir `stateSaved` somente em `transaction.oncomplete`. Emitir apos `put().onsuccess` ainda abre janela para sync remoto antes do commit efetivo do IndexedDB.

Testes novos cobrem os dois casos: chamada concorrente que entra depois da primeira secao critica iniciar e ordem `put -> transaction.oncomplete -> stateSaved`.
...
└── withLock(fn) → Promise
    ...
    └── Sem reentrancia implicita; chamadas aninhadas usam helpers internos sem lock
```

**Decisões de design:**

- Timeout de 30s prevete deadlock permanente
- Sem reentrancia global; isso evita que chamadas concorrentes externas furem a fila
...
4. SyncLock sem reentrancia implicita e com `primarySyncLock` dedicado ao coordenador
...
_Documento gerado em 2026-05-02. Sync hardening completo, 1261 testes passing._
