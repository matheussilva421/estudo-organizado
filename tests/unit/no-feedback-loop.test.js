...
describe('no-feedback-loop (TDD - RED phase)', () => {
  ...
  beforeEach(async () => {
    ...
    vi.doMock('../../src/js/sync/sync-planner.js?v=8.37', () => ({
      planNextSyncAction: vi.fn(() => ({ action: 'none' })),
      ACTIONS: { NONE: 'none', PULL: 'pull', PUSH: 'push' },
    }));

    vi.doMock('../../src/js/sync/firestore-sync-engine.js?v=8.37', async () => {
      const actual = await vi.importActual('../../src/js/sync/firestore-sync-engine.js?v=8.37');
      return {
        ...actual,
    ...
    const coord = await import('../../src/js/sync/sync-coordinator.js?v=8.37');
  ...
  describe('poll-pull does not trigger push cycle', () => {
    ...
    it('after a manual push, polling-pull reads the just-pushed state and does NOT re-push', async () => {
      const { isRemoteStateNewer } = await import('../../src/js/sync/sync-center.js?v=8.37');
