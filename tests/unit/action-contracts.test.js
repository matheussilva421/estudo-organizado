...
describe('data-action contracts', () => {
  ...
  it('uses the extracted calendar view as the runtime calendar owner', async () => {
    ...
    const calendarModule = await import('../../src/js/views/calendar-view.js?v=8.11');

    expect(componentsSource).toContain("from './views/calendar-view.js?v=8.11'");
    expect(mainSource).toContain("import * as calendar_view from './views/calendar-view.js?v=8.11';");
    ...
    expect(calendarSource).toContain("import { esc, getEventStatus, todayStr } from '../utils.js?v=8.11';");
  ...
  it('keeps EstudoApp action targets exported by their owning modules', () => {
    ...
    expect(viewsSource).toMatch(/import\s+\{[^}]*HABIT_TYPES[^}]*\}\s+from\s+['"]\.\/utils\.js\?v=8\.11['"]/s);
  ...
  it('imports the cache invalidators used by revision action handlers', () => {
    ...
    expect(viewsSource).toMatch(/import\s+\{[^}]*invalidatePendingRevCache[^}]*\}\s+from\s+['"]\.\/logic\.js\?v=8\.11['"]/s);
  ...
  it('imports immediate persistence before saving detailed study sessions', () => {
    ...
    expect(registroSource).toMatch(/import\s+\{[^}]*saveStateToDB[^}]*\}\s+from\s+['"]\.\/store\.js\?v=8\.11['"]/s);
