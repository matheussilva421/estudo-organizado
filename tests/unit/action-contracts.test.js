...
describe('data-action contracts', () => {
  ...
  it('uses the extracted calendar view as the runtime calendar owner', async () => {
    ...
    const calendarModule = await import('../../src/js/views/calendar-view.js?v=8.30');

    expect(componentsSource).toContain("from './views/calendar-view.js?v=8.30'");
    expect(mainSource).toContain("import * as calendar_view from './views/calendar-view.js?v=8.30';");
    ...
    expect(calendarSource).toContain("import { esc, getEventStatus, todayStr } from '../utils.js?v=8.30';");
  ...
  it('keeps EstudoApp action targets exported by their owning modules', () => {
    ...
    expect(searchSource).toMatch(/import\s+\{[^}]*HABIT_TYPES[^}]*\}\s+from\s+['"]\.\.\/utils\.js\?v=8\.30['"]/s);
    ...
    expect(registroSource).toMatch(/import\s+\{[^}]*saveStateToDB[^}]*\}\s+from\s+['"]\.\/store\.js\?v=8\.30['"]/s);

    expect(configViewSource).toContain('createExportableState()');
    expect(configViewSource).toMatch(/import\s+\{[^}]*createExportableState[^}]*\}\s+from\s+['"]\.\.\/store\.js\?v=8\.30['"]/s);
  });
  ...
  it('imports the cache invalidators used by revision action handlers', () => {
    ...
    expect(revisaoSource).toMatch(/import\s+\{[^}]*invalidatePendingRevCache[^}]*\}\s+from\s+['"]\.\.\/logic\.js\?v=8\.30['"]/s);
  ...
  it('removes isolated credentials when clearing all app data', () => {
    ...
    expect(storeSource).toContain("from './credentials.js?v=8.30';");
