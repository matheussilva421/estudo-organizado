...
describe('app.js', () => {
  ...
  // =============================================
  // DRIVE POLLING — pause/resume with globalSyncPaused
  // =============================================

  describe('stopDrivePolling()', () => {
    it('clears the drive sync interval when one exists', () => {
      // Start polling first so there is an interval to clear
      store.state.config.globalSyncPaused = false;
      app.startDrivePolling();
      global.clearInterval.mockClear();

      app.stopDrivePolling();
      expect(global.clearInterval).toHaveBeenCalledTimes(1);
    });

    it('is idempotent when no interval is active', () => {
      // _driveSyncInterval should be null — calling stop is safe
      expect(() => app.stopDrivePolling()).not.toThrow();
      // No clearInterval call because no interval existed
      expect(global.clearInterval).not.toHaveBeenCalled();
    });
  });

  describe('startDrivePolling()', () => {
    it('does NOT start polling when globalSyncPaused is true', () => {
      store.state.config.globalSyncPaused = true;
      app.startDrivePolling();
      // setInterval should NOT have been called
      const setIntervalCalls = global.setInterval.mock.calls.length;
      // The count should be unchanged (we cleared the mock, but just check no new calls)
      expect(global.setInterval).not.toHaveBeenCalled();
    });

    it('starts polling when globalSyncPaused is false and Drive is not connected', () => {
      store.state.config.globalSyncPaused = false;
      app.startDrivePolling();
    ...
    it('starts polling when globalSyncPaused is false even without gapi (interval guards itself)', () => {
      store.state.config.globalSyncPaused = false;
      app.startDrivePolling();
    ...
    it('is idempotent when called multiple times', () => {
      ...
      app.startDrivePolling();
      app.startDrivePolling();
      // Each call should clear previous and set new (called 2 times total)
      expect(global.setInterval).toHaveBeenCalledTimes(2);
    });
  });

  describe('module-level event listener for globalSyncPauseChanged', () => {
    it('registers an app:globalSyncPauseChanged listener on document', () => {
      expect(global.document.addEventListener).toHaveBeenCalledWith(
        'app:globalSyncPauseChanged',
        expect.any(Function)
      );
    });
  });
