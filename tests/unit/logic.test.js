  ...
  describe('Ciclo', () => {
    describe('syncCicloToEventos', () => {
      ...
      it('does not shift a deleted slot discipline into the adjacent slot (multi-discipline cycle)', () => {
        ...
        // The deleted slot stays empty...
        expect(day.some((ev) => ev.slotIndex === 0)).toBe(false);
        // ...and its discipline does NOT slide into the next slot (the reported bug).
        expect(day.some((ev) => ev.discId === 'disc_1')).toBe(false);
        // The remaining slots keep their original disciplines.
