...
describe('logic.js', () => {
  ...
  describe('Ciclo', () => {
    describe('syncCicloToEventos', () => {
      ...
      it('sessão concluída SEM seqId reduz a duração do evento gerado (todo estudo conta)', () => {
        const disc = createDisciplina({ id: 'disc_1', nome: 'Direito Administrativo' });
        const edital = createEdital({ disciplinas: [disc] });
        const sessaoLivre = createEvento({
          id: 'ev_livre',
          data: '2026-04-20',
          dataEstudo: '2026-04-20',
          status: 'estudei',
          tempoAcumulado: 4260, // 71 min, sem seqId
          discId: 'disc_1',
        });
        store.setState(
          createBaseState({
        ...
        const autoEvent = store.state.eventos.find((ev) => ev.isAutoGenerated);
        // Previsão e agendador convergem: mesmo restante derivado da mesma fonte
        expect(projection.disc_1).toEqual({ sessoes: 1, minutos: 49 });
        expect(autoEvent.duracao).toBe(49);
      });

      it('etapa completada por sessão livre é concluída e não é reagendada', () => {
        const disc = createDisciplina({ id: 'disc_1', nome: 'Direito Constitucional' });
        const edital = createEdital({ disciplinas: [disc] });
        const sessaoLivre = createEvento({
          id: 'ev_livre',
          ...
          dataEstudo: '2026-04-20',
          status: 'estudei',
          tempoAcumulado: 3 * 3600, // 180 min > alvo de 120, sem seqId
          discId: 'disc_1',
        });
        store.setState(
          createBaseState({
            ...
            planejamento: {
              ...
              disciplinas: ['disc_1'],
              relevancia: {},
              horarios: { diasAtivos: [0, 1, 2, 3, 4, 5, 6] },
              sequencia: [
                { id: 'seq_1', discId: 'disc_1', minutosAlvo: 120, concluido: false, status: 'pendente' },
              ],
              skippedSlots: [],
        ...
        // O cenário das capturas: "100% visual + pendente + reagendado" não pode mais ocorrer
        expect(store.state.planejamento.sequencia[0].status).toBe('concluida');
        expect(store.state.planejamento.sequencia[0].autoConcluida).toBe(true);
        expect(store.state.eventos.filter((ev) => ev.isAutoGenerated)).toEqual([]);
      });
    });
