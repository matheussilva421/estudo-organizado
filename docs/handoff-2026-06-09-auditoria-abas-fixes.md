...
## Rodada 12 (2026-06-10, fix reportado pelo usuário — UI do Histórico desproporcional)

64. **Cache 8.89→8.90** (`210b88a`): query strings do index.html + `APP_VERSION` do sw.js (padrão do `80eac99`; specifiers `?v=8.37` de ES modules são esquema separado, não bumpado). css-architecture+sw-register 37/37; smoke 25/25.
65. **Histórico — UI desproporcional corrigida** (`645d6ec`, screenshot do usuário): três causas combinadas. (a) **`.card { height:100% }`** (existe para nivelar células de grid) fazia cards filhos diretos do `#main-content.main-content-stack` (flex column da viewport) disputarem 100% da altura — toolbar com ~320px de vazio e empty state esticado; neutralizado com `#main-content.main-content-stack > .card { height:auto }` em `cards.css` (afeta TODAS as views com card raiz — é correção, o stretch nunca foi intencional). (b) selects/input da toolbar de filtros **sem `.form-control`** → aparência nativa desproporcional; classe aplicada + `align-items:flex-end` e gap nos labels. (c) **classes `p-12`/`p-24` não existiam** no CSS (só `.p-16`) — toolbar do Histórico e empty states (Histórico e Banca "Nenhum Edital Cadastrado") renderizavam sem padding; utilitários criados (24px == `--space-6` que o `session-empty-state` já tinha, sem mudança visual lá; Banca ganhou o padding que faltava). **Guards novos** em `css-architecture.test.js`: (1) toda classe `p-N` usada em `class="..."` nos templates deve existir no CSS; (2) regra de `height:auto` do stack presente. Atenção a armadilha de teste: `extractCssBlock` casa `.card {` por substring no CSS inlinado — a regra nova precisou ficar em `cards.css` DEPOIS do bloco `.card` (em `layout.css` ela viraria a primeira ocorrência e quebrava o teste de outline). Validação visual: `scripts/screenshot-historico.mjs` (novo; semeia `estudo_state` como o E2E e fotografa empty/populado em `test-results/`). Suíte **1718/1718**; smoke E2E 25/25 ×2.

Observação registrada (não corrigida, possível próximo ciclo): no screenshot do usuário aparecia **"Exibindo 0 de 53 sessões" com Período=Tudo/Disciplina=Todas** — não é o bug de layout: o filtro de edital global (topbar, ex.: PGE-RN) exclui via `eventBelongsToSelectedEdital(..., {allowAll:false})`, mas o denominador "de 53" conta TODOS os eventos estudados (de todos os editais) e o empty state sugere ajustar período/disciplina/busca sem mencionar o edital. Melhorias candidatas: escopar o total ao edital selecionado e/ou citar o filtro de edital na dica do empty state (`historico-view.js:108` e `:140`).
