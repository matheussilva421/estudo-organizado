...
- **Bump de cache**: e AUTOMATICO via hook `pre-commit` (`.githooks/pre-commit`, ativado por `git config core.hooksPath .githooks`, que o `npm install` roda via script `prepare`). Qualquer commit que altere assets de `src/` sem bump recebe o incremento automaticamente em `src/sw.js` (`APP_VERSION`), `src/index.html` (query strings `?v=X.YY`), `src/js/sync/sync-diagnostic.js` (`DIAGNOSTIC_BUILD_VERSION`) e `tests/unit/css-architecture.test.js`. Manual: `npm run bump` (ou `npm run bump -- --set X.YY`); verificacao: `npm run bump:check`; pular o hook: `SKIP_CACHE_BUMP=1 git commit ...`. NAO mexer nos specifiers `?v=8.37` dos imports de modulos ES — sao versionados separadamente e o script nunca os toca.
