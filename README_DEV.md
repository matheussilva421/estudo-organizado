...
- **Bump de cache**: e AUTOMATICO, em duas camadas. (1) **Servidor** — GitHub Action `.github/workflows/auto-cache-bump.yml` roda em todo push para `main`; se o push alterou assets de `src/` sem bump, ele incrementa e envia o commit de bump de volta, e o Cloudflare Pages deploya a versao ja incrementada (nao exige NENHUMA config na maquina de quem commita). (2) **Local (opcional)** — hook `pre-commit` (`.githooks/pre-commit`, ativado por `git config core.hooksPath .githooks`, que o `npm install` roda via script `prepare`) bumpa ja no commit. As duas camadas se complementam: se o hook local ja bumpou, a Action detecta e nao faz nada. O bump atualiza em sincronia `src/sw.js` (`APP_VERSION`), `src/index.html` (query strings `?v=X.YY`), `src/js/sync/sync-diagnostic.js` (`DIAGNOSTIC_BUILD_VERSION`) e `tests/unit/css-architecture.test.js`. Manual: `npm run bump` (ou `npm run bump -- --set X.YY`); verificacao: `npm run bump:check`; pular o hook local: `SKIP_CACHE_BUMP=1 git commit ...`. NAO mexer nos specifiers `?v=8.37` dos imports de modulos ES — sao versionados separadamente e o script nunca os toca.
